...
def split_on_volume(audio, trigger_threshold=0.02, shutoff_threshold=0.005, min_silence_duration=0.6, chunk_size=50):
    rms = audio_to_rms(audio, chunk_size)


    segments = identify_contiguous_segments(rms, trigger_threshold, shutoff_threshold)

    frm = audio.frame_rate
...
def save_segments(audio_filename, segments):
    ...
    for idx, s in enumerate(segments):
        ...
        audio[int(start*1000) : int(end*1000)].export(f'{idx}.mp3', format='mp3')
